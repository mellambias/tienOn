...
class ImageGallery extends HTMLElement {
    ...
    connectedCallback() {
        document.addEventListener('openGallery', event => {
            this.setAttribute('name', event.detail.name);
            this.setAttribute('languageAlias', event.detail.languageAlias);
        });
    ...
    async changeTab(tab) {
        this.shadow
            .querySelectorAll('.tabs-container-menu li')
            .forEach(item => {
        ...
        this.shadow
            .querySelectorAll('.tabs-container-content .tab')
            .forEach(item => {
                item.classList.remove('active');
            });

        this.shadow
            .querySelector(`.tabs-container-content .tab#${tab.id}`)
            .classList.add('active');
    }

    async getThumbnails() {
        try {
            let result = await fetch(`${API_URL}/api/admin/image-gallery`, {
                headers: {
                    Authorization:
            ...
            data.filenames.forEach(filename => {
                html += `
                    <div class="image" data-filename="${filename}">
                        <img src="${API_URL}/api/admin/image-gallery/${filename}" />
                    </div>
                `;
            });
    ...
    async uploadImage(file) {
        try {
            ...
            filenames.forEach(filename => {
                ...
                image.src = `${API_URL}/api/admin/image-gallery/${filename}`;
            ...
            this.shadow
                .querySelector(`li#gallery-content`)
                .classList.add('active');
            ...
            this.shadow
                .querySelector(`.tab#gallery-content`)
                .classList.add('active');
    ...
    async sendDataToForm() {
        let selectedImages = this.shadow.querySelectorAll('.image.selected');
        let imageTitle = this.shadow.querySelector('input[name="title"]').value;
        let imageAlt = this.shadow.querySelector('input[name="alt"]').value;
        let images = {};
        images.name = this.getAttribute('name');
        images.files = [];

        selectedImages.forEach(selectedImage => {
            let file = {};
            file.languageAlias = this.getAttribute('languageAlias');
            file.filename = selectedImage.getAttribute('data-filename');
